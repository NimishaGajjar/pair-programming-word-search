...
const wordSearch = (letters, word) => {
    ...
    // console.log("backwards", backHorJoin);
    console.log(diagonalBackJoin);
    console.log(digonalHorJoin);

    //straight - works
    for (const item in horizontalJoin) {
        if (horizontalJoin.includes(word)) return true;
    }
    //bakwards - works
    ...
    for (const item of diagonalBackJoin) {
        if (diagonalBackJoin.includes(word)) return true;
    }
